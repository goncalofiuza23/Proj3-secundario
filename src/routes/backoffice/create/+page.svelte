...
<script lang="ts">
	...
	function onTitlePtInput(v: string) {
		$formData.titlePt = v;
		if (!slugTouched) $formData.slug = slugify(v);
	}
</script>
			...
			<Form.Field {form} name="titlePt">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input
							{...props}
							bind:value={$formData.titlePt}
							oninput={(e) => onTitlePtInput((e.currentTarget as HTMLInputElement).value)}
						/>
					{/snippet}
				</Form.Control>
				<Form.FieldErrors />
			...
			<Form.Field {form} name="slug" class="mt-4">
				<Form.Control>
					{#snippet children({ props })}
						...
						<Input {...props} bind:value={$formData.slug} oninput={() => (slugTouched = true)} />
					{/snippet}
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
